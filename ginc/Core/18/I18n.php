...
{
    mb_internal_encoding (Ginc::config ('General.charset'));
}
...
class I18n
{
    ...
    public static function translate ($singular, $plural = null, $domain = null, $category = 6, $count = null, $language = null)
    {
        ...
        if (empty ($language))
        {
            if (Input::cookie ('language'))
            {
                $language = Input::cookie ('language');
            }
            else
            {
                $language = Ginc::config ('General.language');
            }
        }
        ...
        $_this->domain = $domain . '_' . Language::$lang;
...
//            Cache::write ($_this->domain, $_this->_domains[$domain][$_this->_lang], '_cake_core_');
    ...
    protected function _bindTextDomain ($domain)
    {
        ...
        foreach (Language::$languagePath as $lang)
        {
            $localeDef = PATH_LOCALE . $lang . DS . $this->category;
            ...
            if ($core)
            {
                $app          = PATH_LOCALE . $lang . DS . $this->category . DS . 'core';
                $translations = false;
                ...
                $file         = PATH_LOCALE . $lang . DS . $this->category . DS . $domain;
                $translations = false;
        ...
        if (isset ($this->_domains[$domain][$this->_lang][$this->category][""]))
        {
            ...
            $this->_domains = self::pushDiff ($this->_domains, $merge);
    ...
    protected function _parseLiteralValue ($string)
    {
        ...
        {
            return self::ascii (array (hexdec ($match[1])));
        }
        return $string;
    }
