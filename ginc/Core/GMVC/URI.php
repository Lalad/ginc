...
class URI
{
    ...
    {
        if (strtoupper (Ginc::config ('General.uri_protocol')) == 'AUTO')
        {
        ...
        $uri = strtoupper (Ginc::config ('General.uri_protocol'));
    ...
    public function _set_uri_string ($str)
    {
        ...
        $str = Security::rmInvisibleCharacters ($str, FALSE);
    ...
    {
        if ($str != '' && Ginc::config ('General.permitted_uri_chars') != '' && Ginc::config ('General.enable_query_strings') == FALSE)
        {
            // preg_quote() in PHP 5.3 escapes -, so the str_replace() and addition of - to preg_quote() is to maintain backwards
            // compatibility as many are unaware of how characters in the permitted_uri_chars will be parsed as a regex pattern
            if ( ! preg_match ("|^[" . str_replace (array ('\\-', '\-'), '-', preg_quote (Ginc::config ('General.permitted_uri_chars'), '-')) . "]+$|i", $str))
            {
    ...
    {
        if (Ginc::config ('General.url_suffix') != "")
        {
            $this->uri_string = preg_replace ("|" . preg_quote (Ginc::config ('General.url_suffix')) . "$|", "", $this->uri_string);
        }
    }
